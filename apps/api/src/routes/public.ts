...
publicRouter.post("/resumes", upload.single("file"), async (req, res, next) => {
  try {
    if (!req.file?.buffer) {
    ...
    void processParseJob(job.id).catch((err) => console.error("processParseJob", job.id, err));
  } catch (e) {
    next(e);
  }
});

publicRouter.get("/resumes/jobs/:id", async (req, res, next) => {
  try {
